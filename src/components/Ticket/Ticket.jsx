...
const Ticket = (props) => {
  const {
    data: {
      ...
      carrier,
      segments: [
        { origin: originTo, destination: destinationTo, date: dateTo, stops: stopsTo, duration: durationTo },
        { origin: originFrom, destination: destinationFrom, date: dateFrom, stops: stopsFrom, duration: durationFrom },
      ],
    },
  } = props;
    ...
    <article className={classes.ticket}>
      ...
      <section className={classes.ticket__way}>
        ...
        <div className={classes.ticket__info}>
          ...
          <p className={classes.ticket__description}>{`${Math.floor(durationTo / 60)}ч ${durationTo % 60}м`}</p>
        </div>
        <div className={classes.ticket__info}>
      ...
      <section className={classes.ticket__way}>
        <div className={classes.ticket__info}>
          ...
          <p className={classes.ticket__description}>
            ...
            {format(add(new Date(dateFrom), { minutes: durationFrom }), 'HH:mm')}
          </p>
        </div>
        <div className={classes.ticket__info}>
          <h3 className={classes.ticket__subtitle}>В пути</h3>
          <p className={classes.ticket__description}>{`${Math.floor(durationFrom / 60)}ч ${durationFrom % 60}м`}</p>
        </div>
        <div className={classes.ticket__info}>
